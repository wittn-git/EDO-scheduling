...
def plot_percentages(input_file_count, output_folder, file_extension):
    ...
    output_file = os.path.join(output_folder, f"percentageplot.{file_extension}")
    plt.savefig(output_file, bbox_inches='tight')

if __name__ == "__main__" :

    if len(sys.argv) < 4:
        print("Usage: python3 PlotPercentages.py <input_file_count> <output_folder> <file_extension>")
        exit(1)
